...
def build_model(pretrained_model_name_or_path: str, device: str) -> CLIPModel:
    """Use same constructor shape as your current CLIPModel"""
    return CLIPModel(pretrained_model_name_or_path=pretrained_model_name_or_path).to(
        device
    )
...
def validate_epoch(
    model: CLIPModel, processor: CLIPProcessor, loader, device: str, tie_margin: float
) -> Dict[str, float]:
...
def setup_training(
    ...
    *,
    ...
    device = build_device()
    model = build_model(pretrained_model_name_or_path, device)
